...
describe('EvidenceDocumentViewer', () => {
  ...
  it('viser tom-tilstand når ingen fil er valgt', () => {
    ...
    expect(
      screen.getByText(/Last opp et forskningsdokument/i)
    ).toBeInTheDocument();
  });

  it('viser PDF i iframe og oppretter object URL', async () => {
    const file = new File(['dummy'], 'review.pdf', {
  ...
  it('viser feil når tekstfilen ikke kan leses', async () => {
    const file = new File([''], 'corrupt.txt', {
      type: 'text/plain',
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Filen kunne ikke forhåndsvises som tekst/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('frigjør object URL ved unmount', () => {
    ...
    const { unmount } = render(
      <EvidenceDocumentViewer file={file} />
    );

...
test('appraisal session cannot be mutated by a different reviewer', async () => {
  const instrumentId = 'jbi-qualitative-2017';
  let state = updateResearchClassification(
    createResearchWorkflowFromText(
      'Aim: explore experiences. Methods: qualitative study.',
      'reviewer-guard.txt',
      'reviewer-guard',
    ),
    classification(instrumentId, 'Kvalitativ'),
  );
  state = verifyResearchClassification(state, 'reviewer-1', true);
  state = selectResearchInstrument(state, instrumentId);
  state = recordScreeningDecision(state, 'reviewer-1', 'INCLUDED');
  ...
  await assert.rejects(
    () =>
      orchestrator.saveResponse(context, {
        itemId: 1,
        answer: 'Yes',
        rationale: 'Unauthorized reviewer attempt',
      }),
    /Reviewer stemmer ikke med appraisal-sesjonen/,
  );
});

test('starting the same appraisal twice reuses the canonical active session', async () => {
  const instrumentId = 'jbi-qualitative-2017';
  let state = updateResearchClassification(
    createResearchWorkflowFromText(
      'Aim: explore experiences. Methods: qualitative study.',
  ...
  const orchestrator = new EvidenceAppraisalOrchestrator();
  const first = await orchestrator.start(state, 'reviewer-1');
  const second = await orchestrator.start(first.workflow, 'reviewer-1');

  assert.equal(second.appraisal.session.id, first.appraisal.session.id);
  assert.equal(second.workflow.appraisalSessions.length, 1);
});

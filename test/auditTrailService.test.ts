...
test('creates a verifiable hash chain', () => {
  const audit = new AuditTrailService();
  audit.append({ actor, action: 'A', subject: { entityType: 'source_record', id: 'r1' } });
  audit.append({ actor, action: 'B', subject: { entityType: 'source_record', id: 'r1' }, detail: { state: 'reviewed' } });
  audit.append({ actor, action: 'C', subject: { entityType: 'source_record', id: 'r1' } });
...
test('detects tampering of an audit entry', () => {
  const audit = new AuditTrailService();
  audit.append({ actor, action: 'A', subject: { entityType: 'source_record', id: 'r1' } });
  audit.append({ actor, action: 'B', subject: { entityType: 'source_record', id: 'r1' } });

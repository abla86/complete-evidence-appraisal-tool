...
public static class ResearchOperationsEndpoints
{
    public static void MapResearchOperationsEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        {
            var rows = await db.ScreeningRecords.AsNoTracking()
        ...
        {
            var latest = await db.ScreeningRecords.AsNoTracking()
                .GroupBy(x => new { x.StudyId, x.IsFullTextStage })
            ...
            var breakdown = fullText.Where(x => x.Status == ScreeningStatus.Excluded)
                .GroupBy(x => x.ExclusionReason ?? "Unspecified")
                .ToDictionary(g => g.Key, g => g.Count(), StringComparer.OrdinalIgnoreCase);

            return Results.Ok(new ScreeningStats(
                title.Count,
                title.Count(x => x.Status != ScreeningStatus.Pending),
                title.Count(x => x.Status == ScreeningStatus.Excluded),
                fullText.Count(x => x.Status != ScreeningStatus.Pending),
                fullText.Count(x => x.Status == ScreeningStatus.Excluded),
                fullText.Count(x => x.Status == ScreeningStatus.Included),
                breakdown));
        });
        ...
        {
            if (request.StudyId == Guid.Empty || string.IsNullOrWhiteSpace(request.Parameter) || string.IsNullOrWhiteSpace(request.Reviewer))
                return Results.BadRequest(new { error = "StudyId, Parameter and Reviewer are required." });
        ...
        endpoints.MapGet("/api/research/operations/extraction/{studyId:guid}", async (Guid studyId, EvidenceDbContext db, CancellationToken cancellationToken) =>
        {
            var rows = await db.DataExtractions.AsNoTracking()
                .Where(x => x.StudyId == studyId)
        ...
        endpoints.MapPost("/api/research/operations/projects/{projectId:guid}/finalize", async (Guid projectId, FinalizeProjectRequest request, EvidenceDbContext db, CancellationToken cancellationToken) =>
        {
            ...
            if (project is null)
            {
                project = new ResearchProjectControlEntity { Id = projectId, Name = request.Name?.Trim() ?? "Research project" };
                db.ResearchProjectControls.Add(project);
            }

            if (project.IsLocked)
                return Results.Conflict(new { error = "Project is already finalized and locked.", project.FinalHash, project.LockedBy, project.LockedAtUtc });

            var payload = new
            {
                project.Id,
                project.Name,
                Screening = await db.ScreeningRecords.AsNoTracking().Where(x => x.StudyId != Guid.Empty).OrderBy(x => x.CreatedAtUtc).ToListAsync(cancellationToken),
                Extractions = await db.DataExtractions.AsNoTracking().OrderBy(x => x.CreatedAtUtc).ToListAsync(cancellationToken),
                Outcomes = await db.ResearchOutcomes.AsNoTracking().Where(x => !x.ProjectId.HasValue || x.ProjectId == projectId).OrderBy(x => x.CreatedAtUtc).ToListAsync(cancellationToken)
            };
            ...
            await db.SaveChangesAsync(cancellationToken);
            await AppendAuditAsync(db, "ResearchProject", project.Id.ToString(), "Finalized", project.LockedBy, new { project.FinalHash }, cancellationToken);

            return Results.Ok(new { projectId, finalized = true, project.FinalHash, project.LockedAtUtc, project.LockedBy });
        });
        ...
        endpoints.MapGet("/api/research/operations/projects/{projectId:guid}", async (Guid projectId, EvidenceDbContext db, CancellationToken cancellationToken) =>
        {
            ...
            return project is null ? Results.NotFound(new { error = "Research project not found." }) : Results.Ok(project);
        });
    ...
    private static async Task AppendAuditAsync(EvidenceDbContext db, string entityType, string entityId, string action, string reviewer, object data, CancellationToken cancellationToken)
    {
        ...
        db.ResearchAudits.Add(new ResearchAuditEntity
        {

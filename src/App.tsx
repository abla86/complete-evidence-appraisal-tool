...
export default function App() {
  ...
  const handleImportArticles = (importedArticles: ArticleAppraisal[], mode: 'append' | 'replace') => {
    ...
    else { const existingIds = new Set(articles.map(p => p.id)); const newItems: ArticleAppraisal[] = []; importedArticles.forEach(item => { let uniqueId = item.id; if (!uniqueId || existingIds.has(uniqueId)) uniqueId = generateArticleId('art'); existingIds.add(uniqueId); newItems.push({ ...item, id: uniqueId }); }); setArticles(prev => [...newItems, ...prev]); if (newItems.length) { setSelectedArticleId(newItems[0].id); setSelectedInstrumentId(newItems[0].instrumentId || ''); } }
  };
  const handleSelectArticle = (id: string) => { setSelectedArticleId(id); setActiveTab('details'); };
  const handleEditArticle = (article: ArticleAppraisal) => { if (article.instrumentId) setSelectedInstrumentId(article.instrumentId); setArticles(prev => prev.map(a => a.id === article.id ? article : a)); setEditingArticle(article); setSelectedArticleId(article.id); setActiveTab('instrumentinfo'); };
  const handleNewArticle = () => { setEditingArticle(null); setSelectedInstrumentId(''); setActiveTab('instrumentinfo'); };
  const handleSaveArticle = (savedArticle: ArticleAppraisal) => { setArticles(prev => { const idx = prev.findIndex(a => a.id === savedArticle.id); if (idx >= 0) { const next = [...prev]; next[idx] = savedArticle; return next; } return [savedArticle, ...prev]; }); setSelectedArticleId(savedArticle.id); setEditingArticle(null); setSelectedInstrumentId(savedArticle.instrumentId || ''); setActiveTab('details'); };
  const currentArticle = articles.find(a => a.id === selectedArticleId) || articles[0];
  const hardResetApp = () => { if (typeof localStorage !== 'undefined') localStorage.clear(); if (typeof sessionStorage !== 'undefined') sessionStorage.clear(); window.location.reload(); };
  ...
  return (
    <ToastProvider>
      <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 font-sans selection:bg-teal-100 selection:text-teal-900">
        ...
        <StudioStateProvider value={{ articles, setArticles, references: referenceRecords, setReferences: setReferenceRecords, pipelineState, setPipelineState, selectedArticleId, setSelectedArticleId }}>
          <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8">
            ...
            {Boolean(selectedInstrumentId) && activeTab === 'instrumentinfo' ? <UniversalAppraisalView studyId={currentStudyId || 'new-study'} studyDesign={currentArticle?.design || ''} initialInstrumentId={selectedInstrumentId} reviewerId={appraisalReviewerId} onSaved={saveAppraisalSession} /> : <>
              ...
              {activeTab === 'evaluate' && <UniversalAppraisalView studyId={currentStudyId || 'new-study'} studyDesign={currentArticle?.design || ''} initialInstrumentId={selectedInstrumentId || currentArticle?.instrumentId || ''} reviewerId={appraisalReviewerId} onSaved={saveAppraisalSession} />}
        ...
        <AutosaveModal isOpen={isAutosaveModalOpen} onClose={() => setIsAutosaveModalOpen(false)} articles={articles} onRestoreArticles={(restoredArticles) => { setArticles(restoredArticles); setSelectedArticleId(restoredArticles[0]?.id || ''); }} />
        <GdprPrivacyCenterModal isOpen={isPrivacyCenterOpen} onClose={() => setIsPrivacyCenterOpen(false)} articles={articles} onVaultPurged={() => { setArticles([]); setReferenceRecords([]); setAppraisalSessions([]); setSelectedArticleId(''); setEditingArticle(null); setPipelineState(new EvidencePipelineService().create('workspace')); }} />
      </div>
    </ToastProvider>
  );
}

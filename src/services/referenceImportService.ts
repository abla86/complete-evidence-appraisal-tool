...
function mapAuthors(value?: string): string {
  return (value || '').split(/\s+and\s+/i).map(v => v.trim()).filter(Boolean).join('; ');
}
...
function parseCslJson(input: string): ParsedReference[] {
  ...
  return list.map(value => {
    ...
    return {
      ...
      authors: normalizeAuthors(Array.isArray(item.author) ? item.author.map(author => {
...
function parseEndnoteXml(input: string): ParsedReference[] {

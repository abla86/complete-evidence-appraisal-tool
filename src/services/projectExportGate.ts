...
export function evaluateProjectExportGate(input: ProjectExportGateInput): ProjectExportGateResult {
  ...
  for (const synthesis of input.synthesis ?? []) {
    const validation = validateSynthesis(
      synthesis,
      input.appraisal.map(item => ({ id: item.id, studyId: item.studyId, locked: item.locked })),
      input.evidence,
      input.claims,
    );
  ...
  const verifiedSourceIds = new Set(
    input.references
      .filter(reference => reference.verification === 'VALIDATED')
  ...
  for (const item of input.quality) {
    ...
    if (session && session.reviewerId !== item.reviewerId) {
      blockers.push(`Quality-vurdering ${item.id} har annen reviewer enn appraisal-session.`);
    }
  }
  ...
  const invalidQualityLinks = input.quality.filter(
    item => !evidenceIds.has(item.evidenceId) || !appraisalIds.has(item.appraisalSessionId),
  );
  if (invalidQualityLinks.length > 0) {
    blockers.push(`${invalidQualityLinks.length} quality-vurdering(er) peker til manglende evidence eller appraisal-session.`);
  }

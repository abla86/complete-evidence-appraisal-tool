...
import type { ResearchEvidenceRecord } from '../types/workflow.contracts';
...
export class InMemoryAppraisalWorkflowStore implements AppraisalWorkflowStore {
  ...
  public listByStudy(studyId: string): AppraisalWorkflowRecord[] {
    return [...this.records.values()].filter(record => record.researchStudyId === studyId);
  }
  ...
  public listAll(): AppraisalWorkflowRecord[] {
...
function isHumanVerifiedEvidence(item: ResearchEvidenceRecord): boolean {
  return item.source === 'HUMAN_VERIFIED' &&
    item.verifiedByResearcher === true &&
    Boolean(item.verifiedBy?.trim()) &&
    Boolean(item.verifiedAt?.trim());
}
...
function assertWorkflowReady(workflow: WorkflowState, instrumentId: string): void {
  ...
  const hasIncluded = workflow.screening.some(
    record => record.studyId === workflow.studyId && record.decision === 'INCLUDED',
  );
  if (!hasIncluded) throw new Error('INCLUDED screening decision required before appraisal.');

  if (!workflow.research.classificationVerified) {
    throw new Error('Human verification of document classification is required before appraisal.');
  }

  const selected = workflow.research.selectedInstrumentId?.trim();
  if (!selected || selected !== instrumentId.trim()) {
    throw new Error('Selected appraisal instrument is not consistent with workflow gating.');
  }

  const recommendation = workflow.research.evidenceBundle.gating.instrumentRecommendation?.trim();
  if (!recommendation || recommendation !== instrumentId.trim()) {
    throw new Error('Appraisal instrument recommendation is missing or inconsistent.');
  }

  const decision = decideAppraisalLaunch(workflow.studyDesign, instrumentId);
...
function assertPayload(payload: ResearchAppraisalPayload, reviewerId: string): void {
  ...
  if (!Array.isArray(payload.evidence) || payload.evidence.length === 0) {
    throw new Error('Appraisal payload contains no human-verified evidence.');
  }

  const ids = payload.evidence.map(item => item.id.trim());
  if (ids.some(id => !id) || new Set(ids).size !== ids.length) {
    throw new Error('Appraisal payload contains invalid or duplicate evidence ids.');
  }

  if (payload.evidence.some(item => !isHumanVerifiedEvidence(item))) {
...
function assertCanonicalEvidence(
  ...
  const canonical = getCanonicalEvidence(workflow);
  const canonicalById = new Map(canonical.map(item => [item.id, item]));
  const payloadIds = payload.evidence.map(item => item.id.trim());

  if (workflow.research?.document.id !== payload.document.id.trim()) {
    throw new Error('Appraisal payload document does not match the canonical research document.');
  }
  ...
  for (const item of payload.evidence) {
    ...
    if (source.studyId !== workflow.studyId || source.documentId !== payload.document.id.trim()) {
      throw new Error(`Evidence ${item.id} has invalid study or document provenance.`);
    }
    if (!isHumanVerifiedEvidence(source)) {
      throw new Error(`Evidence ${item.id} is no longer human verified in the canonical research workflow.`);
    }
  }
...
function assertRecordEvidence(record: AppraisalWorkflowRecord, workflow: WorkflowState): void {
  ...
  for (const evidenceId of record.evidenceIds) {
    ...
    if (!source || !isHumanVerifiedEvidence(source)) {
      throw new Error(`Appraisal session references evidence that is missing or no longer human verified: ${evidenceId}`);
    }
  }
}
...
function findActiveSession(payload: ResearchAppraisalPayload, reviewerId: string) {
  ...
  const instrument = payload.instrumentId.trim();
  const reviewer = reviewerId.trim();
  return appraisalWorkflowStore
    .listByStudy(study)
    .find(record =>
      record.instrumentId === instrument &&
      record.session.instrumentId === instrument &&
      record.session.reviewerId === reviewer &&
      !record.session.locked,
    );
}
...
export async function createAppraisalFromResearch(
  ...
  if (existing) {
    assertRecordEvidence(existing, workflow);
    return existing;
  }
  ...
  await appendAuditEntry({
    actor: { id: reviewerId.trim(), role: 'reviewer' },
    action: 'appraisal.session.created',
    subject: { entityType: 'appraisal-session', id: session.id },
    detail: { studyId: session.studyId, instrumentId: session.instrumentId, evidenceIds: record.evidenceIds },
  });
...
export async function createAndAttachAppraisal(
  ...
  if (existing) {
    assertRecordEvidence(existing, workflow);
    return { record: existing, workflow: syncToResearchWorkflow(existing.session) };
  }
...
export function getAppraisalWorkflowRecord(sessionId: string) {
  return appraisalWorkflowStore.get(sessionId.trim());
}

export async function changeAppraisalInstrument(
  sessionId: string,
  instrumentId: string,
  reviewerId: string,
): Promise<AppraisalWorkflowRecord> {
  ...
  await appendAuditEntry({
    actor: { id: reviewerId.trim(), role: 'reviewer' },
    action: 'appraisal.instrument.changed',
    subject: { entityType: 'appraisal-session', id: record.session.id },
    detail: { previousInstrumentId: record.session.instrumentId, instrumentId: instrument.id, responsesReset: true },
  });
  await evidenceEventBus.emit('appraisal.instrument.changed', {
    studyId: session.studyId,
    sessionId: session.id,
    instrumentId: session.instrumentId,
    reviewerId: session.reviewerId,
...
export async function recordAppraisalResponse(
  ...
  if (!instrument || (instrument.questions ?? []).every(q => String(q.id) !== itemId)) {
    throw new Error('itemId finnes ikke i valgt appraisal-instrument.');
  }

  if (response.evidence?.sourceId) {
    const sourceId = response.evidence.sourceId.trim();
    if (!record.evidenceIds.includes(sourceId)) {
      throw new Error(`Evidence reference ${sourceId} is not attached to this appraisal session.`);
    }
    const source = getCanonicalEvidence(workflow).find(item => item.id === sourceId);
    if (!source || !isHumanVerifiedEvidence(source)) {
      throw new Error(`Evidence reference ${sourceId} is missing or no longer human verified.`);
    }
  ...
  const session = upsertAppraisalResponse(record.session, {
    ...
    evidence: response.evidence
      ? { ...response.evidence, sourceId: response.evidence.sourceId?.trim() }
      : undefined,
  });
  syncToResearchWorkflow(session);
  ...
  await appendAuditEntry({
    ...
    action: 'appraisal.response.updated',
    subject: { entityType: 'appraisal-session', id: sessionId },
    detail: { itemId, answer: response.answer, evidenceSourceId: response.evidence?.sourceId },
  });
  return saved;
}
...
export async function finalizeAppraisal(sessionId: string): Promise<AppraisalWorkflowRecord> {
  ...
  await appendAuditEntry({
    ...
    subject: { entityType: 'appraisal-session', id: session.id },
    detail: { studyId: session.studyId, instrumentId: session.instrumentId, responseCount: session.responses.length, evidenceIds: record.evidenceIds },
  });

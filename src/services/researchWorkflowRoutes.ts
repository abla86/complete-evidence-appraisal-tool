...
function requireWorkflow(studyId: string, projectId?: string): WorkflowState {
  ...
  if (!workflow) throw new Error(`Research workflow not found: ${normalizedStudyId}`);
  return workflow;
}

function save(workflow: WorkflowState): WorkflowState { return researchWorkflowStore.save(workflow); }

/**
 * Ownership boundary for multi-user research data.
 * Legacy workflows without ownerId remain readable only when the authenticated
 * reviewer is already recorded in the workflow. New workflows always receive ownerId.
 */
export function canAccessResearchWorkflow(workflow: WorkflowState, reviewerId: string): boolean {
  const reviewer = reviewerId.trim();
  if (!reviewer) return false;
  if (workflow.ownerId?.trim()) return workflow.ownerId.trim() === reviewer;
  return workflow.screening.some(record => record.reviewerId === reviewer)
    || workflow.appraisalSessions.some(session => session.reviewerId === reviewer);
}

function assertWorkflowAccess(workflow: WorkflowState, reviewerId: string): void {
  if (!canAccessResearchWorkflow(workflow, reviewerId)) throw new Error('Research workflow access denied.');
}

export function registerResearchWorkflowRoutes(app: { get: Function; post: Function }): void {
  app.get('/api/research-workflows', (req: Request, res: Response) => {
    try {
      const reviewerId = reviewerIdForRequest(req);
      const workflows = researchWorkflowStore.list().filter(workflow => canAccessResearchWorkflow(workflow, reviewerId));
  ...
  app.post('/api/research-workflows', (req: Request, res: Response) => {
    try {
      ...
      const fileName = typeof req.body?.fileName === 'string' && req.body.fileName.trim() ? req.body.fileName : 'document.txt';
      if (typeof text !== 'string' || !text.trim()) return sendError(res, 400, 'text is required');
      const workflow = createResearchWorkflowFromText(text, fileName, req.body?.studyId);
      const existing = researchWorkflowStore.get(workflow.studyId);
      if (existing) {
        if (!canAccessResearchWorkflow(existing, reviewerId)) return sendError(res, 403, 'Research workflow access denied');
        return sendError(res, 409, 'Research workflow already exists');
      }
      const saved = save({ ...workflow, ownerId: reviewerId });
      return res.status(201).json({ success: true, workflow: saved, evidenceSummary: getResearchEvidenceSummary(saved) });
  ...
  app.post('/api/research-workflows/:studyId/appraisal/ready', (req: Request, res: Response) => {
    ...
    } catch (error) { return sendError(res, 409, error); }
  });

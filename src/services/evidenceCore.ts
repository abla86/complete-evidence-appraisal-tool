...
export function finalizeAssessment(a: Assessment, actorId: string): Assessment {
  ...
  const errors = validateAssessment(a); if (errors.length) throw new Error(errors.join(' '));
...
export function verifyImmutableAssessment(a: Assessment): boolean { return a.lifecycle === 'FINALIZED' && !!a.immutableHash && sha256({ ...a, immutableHash: undefined }) === a.immutableHash; }
export function appendAuditEvent(events: AuditEvent[], assessmentId: string, actorId: string, action: string, payload: unknown): AuditEvent {
  const previousEventHash = events.length ? events[events.length - 1].eventHash : null; const payloadHash = sha256(payload); const timestamp = new Date().toISOString();
  const eventHash = sha256({ assessmentId, actorId, action, timestamp, payloadHash, previousEventHash });
  const event = { id: randomUUID(), assessmentId, actorId, action, timestamp, payloadHash, previousEventHash, eventHash }; events.push(event); return event;
}
export function verifyAuditChain(events: AuditEvent[]): boolean {
  let previous: string | null = null;
  for (const event of events) { if (event.previousEventHash !== previous) return false; const expected = sha256({ assessmentId: event.assessmentId, actorId: event.actorId, action: event.action, timestamp: event.timestamp, payloadHash: event.payloadHash, previousEventHash: event.previousEventHash }); if (expected !== event.eventHash) return false; previous = event.eventHash; }
  return true;
}
export function classifyInput(text: string): 'PUBLIC_RESEARCH_DATA'|'PERSONAL_DATA_RISK'|'UNKNOWN' {
  const t = text.toLowerCase(); if (/fødselsnummer|personnummer|social security|national id|phone|telefonnummer|adresse|address|email|e-post/.test(t)) return 'PERSONAL_DATA_RISK'; if (/doi|abstract|randomized|randomisert|systematic review|systematisk oversikt|guideline|retningslinje/.test(t)) return 'PUBLIC_RESEARCH_DATA'; return 'UNKNOWN';
}
export function enforceAiBoundary<T extends Record<string, unknown>>(proposal: T): T & { _status: 'AI_CANDIDATE' } { return { ...proposal, _status: 'AI_CANDIDATE' }; }

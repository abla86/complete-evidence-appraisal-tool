...
export function createAssessment(input: Omit<Assessment, 'id'|'createdAt'|'updatedAt'|'lifecycle'>): Assessment {
  if (!input.studyId.trim() || !input.studyTitle.trim() || !input.instrumentId.trim() || !input.instrumentVersion.trim() || !input.reviewerId.trim()) {
    throw new Error('studyId, studyTitle, instrumentId, instrumentVersion og reviewerId er påkrevd.');
  }
...
export function verifyImmutableAssessment(a: Assessment): boolean {
  if (a.lifecycle !== 'FINALIZED' || !a.immutableHash) return false;
  return sha256({ ...a, immutableHash: undefined }) === a.immutableHash;
}
...
export function appendAuditEvent(events: AuditEvent[], assessmentId: string, actorId: string, action: string, payload: unknown): AuditEvent {
  ...
  const event = { id: randomUUID(), assessmentId, actorId, action, timestamp, payloadHash, previousEventHash, eventHash };
  events.push(event);
  return event;
}

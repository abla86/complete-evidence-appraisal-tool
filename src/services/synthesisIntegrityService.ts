...
export function validateSynthesis(
  ...
  for(const input of synthesis.inputs){
    ...
    if((synthesis.type === 'QUALITATIVE_THEMATIC' || synthesis.type === 'META_AGGREGATION') && (input.value !== undefined || input.standardError !== undefined || input.confidenceInterval !== undefined)) warnings.push(`Kvalitativt synteseinput ${input.id} inneholder kvantitativ metadata; kontroller at dette ikke blandes inn i kvalitativ syntese.`);

...
export function validatePRISMAFlow(flow: PRISMAFlow): PRISMAValidation {
  ...
  if (identified !== id.recordsFromDatabases + id.recordsFromOtherSources) errors.push('totalIdentified må være summen av database- og øvrige kilder.');
  if (id.duplicatesRemoved > identified) errors.push('duplicatesRemoved kan ikke overstige identifiserte poster.');
  if (s.recordsScreened !== identified - id.duplicatesRemoved) errors.push('recordsScreened må samsvare med identifiserte poster etter deduplisering.');
  if (s.recordsExcluded > s.recordsScreened) errors.push('recordsExcluded kan ikke overstige recordsScreened.');
  if (e.fullTextsAssessed !== s.recordsScreened - s.recordsExcluded) errors.push('fullTextsAssessed må samsvare med poster etter title/abstract-screening.');
  ...
  if (i.studiesFinalSynthesis > i.studiesQualityAssessment) warnings.push('studiesFinalSynthesis er større enn studiesQualityAssessment; kontroller om alle inkluderte studier faktisk er kvalitetsvurdert.');

...
function requireReviewer(req: Request): string {
...
export function registerAppraisalWorkflowApi(app: { get: Function; post: Function }): void {
  ...
  app.get('/api/research-workflow/:studyId/appraisal/sessions', (req: Request, res: Response) => {
    ...
    if (!workflow) return res.status(404).json({ success: false, error: 'Workflow not found' });
    return res.json({ success: true, sessions: appraisalWorkflowStore.listByStudy(studyId), workflow, research: getResearchEvidenceSummary(workflow) });
  });
  ...
  app.post('/api/appraisal/:sessionId/sync', async (req: Request, res: Response) => {
    try {
      ...
      if (incoming.studyId !== record.session.studyId || incoming.instrumentId !== record.session.instrumentId || incoming.instrumentVersion !== record.session.instrumentVersion || incoming.reviewerId !== record.session.reviewerId) {
  ...
  app.get('/api/appraisal/:sessionId/validation', (req: Request, res: Response) => {
    try { return res.json({ success: true, validation: validateAppraisal(req.params.sessionId.trim()) }); }
    catch (error) { return res.status(404).json({ success: false, error: error instanceof Error ? error.message : 'Appraisal session not found' }); }
  });

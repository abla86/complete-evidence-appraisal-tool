...
export const DualReviewView: React.FC<DualReviewViewProps> = ({
  ...
  const reviewer2Items = useMemo(() => {
    ...
    return dualArticle.items.map(it => ({ ...it, status: reviewer2Draft[it.questionId] || 'Uklart', justification: reviewer2Draft[it.questionId] ? 'Reviewer 2: uavhengig vurdering registrert.' : 'Reviewer 2: ikke vurdert ennÃ¥.' }));
  ...
  const handleSaveConsensus = () => {
    if (!reviewer2Complete) { showToast('Reviewer 2 mÃ¥ vurdere alle kriterier fÃ¸r konsensus kan registreres.', 'warning'); return; }

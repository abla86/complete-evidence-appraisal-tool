...
export const PdfEvidenceWorkspace: React.FC<Props> = ({ references, onChange }) => {
  ...
  const attach = async (file: File) => {
    if (!selected) { setMessage('Velg en referanse fÃ¸rst.'); return; }
    try {
      ...
      setMessage(`PDF koblet til Â«${selected.title}Â». SHA-256 er beregnet lokalt.`);
    } catch (error) {
  ...
  return <section className="space-y-4 bg-white border border-slate-200 rounded-2xl p-5">
    <div><div className="text-[10px] uppercase tracking-wide text-slate-500">Evidence Appraisal Â· Fulltekst</div><h2 className="text-xl font-bold font-serif">PDF â†’ kilde</h2><p className="text-sm text-slate-600 mt-1">Velg referanse og legg til fulltekst. Filen knyttes til den kanoniske Reference Hub-posten.</p></div>
    ...
    {selected && <div className="rounded-xl bg-slate-50 border border-slate-200 p-4"><div className="text-sm font-bold">{selected.title}</div><div className="text-xs text-slate-500 mt-1">PDF-er: {pdfs.length}{selectedFile ? ` Â· Sist lagt til: ${selectedFile}` : ''}</div><div className="space-y-2 mt-3">{pdfs.map(pdf => <div key={pdf.id} className="rounded-lg bg-white border border-slate-200 p-3 text-xs"><div className="font-semibold">{pdf.name}</div><div className="text-slate-500 mt-1">SHA-256: {pdf.sha256 || 'mangler'}</div></div>)}</div></div>}
  </section>;
};

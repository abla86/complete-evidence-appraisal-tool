...
export const ResearchSearchView: React.FC<ResearchSearchProps> = ({ onImportArticle, existingArticles }) => {
  ...
  const handleSearch = async (overrideQuery?: string, overrideDb?: string) => {
    ...
    } catch (err: any) { console.error('Search error:', err); showToast(`Søkefeil: ${err.message || 'Kunne ikke kontakte databasen'}`, 'error'); }
  ...
  const handleVerifyDoi = async (doi: string) => {
    ...
    } catch (err: any) { showToast(`Kildeverifisering feilet: ${err.message || 'ukjent feil'}`, 'error'); }
  ...
  const handleImport = (rec: OpenResearchRecord) => {
    ...
    onImportArticle({ title: rec.title, authors: rec.authors, journal: rec.journal, publicationYear: parseInt(rec.year, 10) || new Date().getFullYear(), year: parseInt(rec.year, 10) || new Date().getFullYear(), doi: rec.doi || '', doiUrl: rec.doi ? `https://doi.org/${Apa7CitationService.cleanDoi(rec.doi)}` : '', shortCitation: formattedApa.shortCitation, apaReference: formattedApa.plainText, abstract: rec.abstract || '', methodology: 'Kvalitativ / Empirisk analyse (Klar for sjekkliste)', studyDesign: rec.studyTypeHint || 'Kvalitativ / Observasjonell', epistemology: 'Uavklart', overallVerdict: 'Vurder videre', instrumentId: 'jbi-qualitative-2017' });

...
export const ValidationDashboardView: React.FC = () => {
  ...
  const instrumentCards: ValidationDashboardInstrumentCard[] = useMemo(() => {
    ...
    return MASTER_INSTRUMENTS_REGISTRY.map(instrument => {
      ...
      return {
        ...
        statusExplanation: results.length ? `${passed}/${results.length} kjÃ¸rte kontrakttester bestÃ¥tt; auditstatus ${status}.` : `Ingen eksplisitt runtime-test registrert; auditstatus ${status}.`,
  ...
  const releaseGateChecks: ReleaseGateCheck[] = useMemo(() => [
    { id: 'RG-01', title: 'Alle kontrakttester bestÃ¥tt', passed: testSummary.allPassed, details: `${testSummary.passedTests}/${testSummary.totalTests}` },
  ...
  return (
    <section className="space-y-6 pb-12">
      <header className="bg-white border border-slate-200 rounded-2xl p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Status hentes fra kjÃ¸rte kontrakttester og faktisk instrumentaudit. Ingen status markeres som bestÃ¥tt uten underliggende resultat.</p>
          </div>
          <button type="button" onClick={runTests} disabled={isRunning} className="inline-flex items-center gap-2 rounded-xl bg-teal-800 text-white px-4 py-2 text-sm font-bold disabled:opacity-50">
            ...
            {isRunning ? 'KjÃ¸rerâ€¦' : 'KjÃ¸r validering'}
          </button>
        </div>
      </header>
      ...
      <div className="grid md:grid-cols-4 gap-3">
        ...
        <Metric label="BestÃ¥tt" value={testSummary.passedTests} />
          ...
          {instrumentCards.map(card => (
            <article key={card.instrumentId} className="rounded-xl border border-slate-200 p-4 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div><h3 className="font-bold text-sm">{card.instrumentName}</h3><p className="text-[11px] text-slate-500">{card.instrumentId} Â· v{card.version}</p></div>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4">
        <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="SÃ¸k i testresultaterâ€¦" className="w-full rounded-xl border border-slate-300 pl-9 pr-3 py-2 text-sm" /></div>
          <select value={selectedLevel} onChange={e => setSelectedLevel(e.target.value as typeof selectedLevel)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm"><option value="ALL">Alle nivÃ¥er</option><option value="LEVEL_1_UNIT">Level 1</option><option value="LEVEL_2_INTEGRATION">Level 2</option><option value="LEVEL_3_REFERENCE">Level 3</option></select>
        </div>
        <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
          {filteredResults.map(result => (
            <div key={result.ruleId} className="p-3 flex items-start gap-3 text-xs">
              ...
              <div className="min-w-0 flex-1"><div className="font-bold text-slate-900">{result.ruleTitle}</div><div className="text-slate-500 mt-0.5">{result.ruleId} Â· {result.level}</div><div className="text-slate-600 mt-1">{result.details}</div></div>
